    ...
    if request.is_ajax() and request.method == "POST":
        ...
        if conversao == 'A':
            if prod != '':
                ...
                produtos = Produto.objects.filter(cdsubgrupo_cdGrupo=cdgp, ativo='A').values('cdproduto','nmproduto', 'cdunidade_nmUnidade', 'percPerda')
        else:
            if prod != '':
                ...
                produtos = Produto.objects.filter(cdsubgrupo_cdGrupo=cdgp, ativo='A').values('cdproduto','nmproduto', 'cdunipadrao_nmUnidade', 'percPerda')
...
def movimento(request):
    ...
    if request.is_ajax() and request.method == "POST":
        ...
        if flagCadastro == 0:
            ...
            dados = Itemmovimentado.objects.filter(cdmovimentacao_id=idMov).values('cdmovimentacao_id','cditemmovimentado', 'cdproduto_nmproduto','cdunidade_nmUnidade', 'valor', 'valorLiquido').order_by('cditemmovimentado')
